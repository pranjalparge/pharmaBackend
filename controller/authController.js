...
exports.register = async (req, res) => {
  const { name, email, password, siteName, siteLocation, siteAddress  } = req.body;
  ...
  try {
    ...
    if (userExists) {
      return res.status(200).json({ status: false,  message: 'Client already exists' });
    }
    const user = new User({ name, email, password, siteName, siteLocation, siteAddress  });
    await user.save();
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ status :true, "token" : token, uid: user.uid, "user": { "name" : user.name, "email" : user.email, "role" : user.role, "site" : user.siteName, "site_address" : user.siteAddress, "site_location" : user.siteLocation } });
  } catch (error) {
    return res.status(500).json({ status: false,  message: error.message });
  }
};
